...
class ReleaseManager:
    ...
    SUBFIX_VERSION_FILE = '../src/subfix/__init__.py'
    DEFAULT_CHANGE = 'patch'
    DEFAULT_IS_BETA = False

    def _get_current_version(self):
        """
        gets current subfix version from subfix package.

        :returns: tuple(int major, int minor, int patch, str beta)
        :rtype: tuple
        """

        with io.open(self.SUBFIX_VERSION_FILE, 'rt', encoding='utf8') as version_file:
    ...
    def _set_version(self, old_version, new_version):
        ...
        with open(self.SUBFIX_VERSION_FILE, 'r') as file:
            file_data = file.read()

        file_data = file_data.replace(formatted_old, formatted_new)

        with open(self.SUBFIX_VERSION_FILE, 'w') as file:
    ...
    def _make(self):
        ...
        self._execute_command('cd .. ; python3.8 setup.py sdist bdist_wheel')
    ...
    def _upload(self):
        ...
        self._execute_command('cd .. ; python3.8 -m twine upload dist/*')
